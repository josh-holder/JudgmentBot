...
from JudgementAgent import JudgementAgent
...
DEFAULT_AGENTS = [JudgementAgent(),JudgementAgent(),JudgementAgent(),JudgementAgent()]

class JudgementGame(object):
    ...
    def playRound(self):
        ...
        """
        self.resetAgents()
        trump = self.round % len(SUIT_ORDER)
        hand_size = self.hand_sizes[self.round]

        if self.game_verbose: print("~~ Round {}, Hand Size {}, Trump {} ~~".format(self.round,hand_size,SUIT_ORDER[trump]))
...
if __name__ == "__main__":
    jg = JudgementGame(game_verbose=1)
    jg.agents[0].id=0
    jg.agents[1].id=1
    jg.agents[2].id=2
    jg.agents[3].id=3
